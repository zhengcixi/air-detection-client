...
//串口发送缓存区 
__align(8) u8 USART2_TX_BUF[USART2_MAX_SEND_LEN]; 	//发送缓冲,最大USART2_MAX_SEND_LEN=1024字节
#ifdef USART2_RX_EN  //如果使能了接收   	  
//串口接收缓存区 	
u8 USART2_RX_BUF[USART2_MAX_RECV_LEN]; 				//接收缓冲,最大USART2_MAX_RECV_LEN=1024个字节.
...
{  
	...
	USART_DMACmd(USART2,USART_DMAReq_Tx,ENABLE);  	//使能串口2的DMA发送请求
	...
	USART2_RX_STA=0;		//串口接收数据状态寄存器清零（自定义变量）
	TIM4_Set(0);			//关闭定时器4
}
...
注：vsprintf(char* string, char* format, va_list parm)介绍
    功能：使用参数列表发送格式化输出到字符串中
	参数：string:存放字符串；format:参数的格式；ap:参数
——————————————————————————————*/
void u2_printf(char* fmt,...)  
{  
	va_list ap;
	va_start(ap, fmt);
	vsprintf((char*)USART2_TX_BUF, fmt, ap);  //示例：若传入参数是："%s\r\n","AT" 则==>USART2_TX_BUF="AT\r\n"
	va_end(ap);
...
void UART_DMA_Config(DMA_Channel_TypeDef*DMA_CHx,u32 cpar,u32 cmar)
{
	DMA_InitTypeDef DMA_InitStructure;
 	RCC_AHBPeriphClockCmd(RCC_AHBPeriph_DMA1, ENABLE);	//设置DMA时钟
